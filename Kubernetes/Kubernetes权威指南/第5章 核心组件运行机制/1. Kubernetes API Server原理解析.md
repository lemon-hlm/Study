...
从本质上看，API Server与常见的**MIS**或**ERP**系统中的**DAO模块**类似，可以将**主要处理逻辑**视作对**数据库表的CRUD(Create\<创建> Read/Retrieve\<读取> Update\<更新> Delete\<删除>)操作**。这里解读**API Server**中**资源对象的List\-Watch机制**。
