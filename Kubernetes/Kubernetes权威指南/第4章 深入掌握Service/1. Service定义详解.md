...
- [2 Service的基本用法](#2-service的基本用法)
  - [2.1 多端口Service](#21-多端口service)
  - [2.2 外部服务Service](#22-外部服务service)
- [3 Headless Service](#3-headless-service)
...
Kubernetes提供了**Headless Service**来实现这种功能，即不为Service设置ClusterIP（入口IP地址），仅通过Label Selector将后端的Pod列表返回给调用的客户端。例如：
