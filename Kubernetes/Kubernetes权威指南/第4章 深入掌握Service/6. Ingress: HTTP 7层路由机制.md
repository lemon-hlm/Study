...
在Ingress生效之前，需要先将webapp服务部署完成。同时需要注意Ingress中path的定义，需要与后端真实Service提供的path一致，否则将被转发到一个不存在的path上，引发错误。这里以第1章的例子为例，假设myweb服务已经部署完毕并正常运行，myweb提供的Web服务的路径也为/demo。
