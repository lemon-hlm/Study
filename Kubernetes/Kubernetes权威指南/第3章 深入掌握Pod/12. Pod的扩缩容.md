...
- 对于**CPU使用率**，在**target参数**中设置**averageUtilization**定义目标**平均CPU使用率**。对于**内存资源**，在target参数中设置AverageValue定义目标**平均内存使用值**。指标数据可以通过API“metrics.k8s.io”进行查询，要求预先启动Metrics Server服务。
