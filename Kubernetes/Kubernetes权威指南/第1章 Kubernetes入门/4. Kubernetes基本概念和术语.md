...
**Pod的Endpoint地址**会随着**Pod的销毁和重新创建**而发生**改变**，因为**新Pod的IP地址与之前旧Pod的不同**。而**Service一旦被创建**，Kubernetes就会自动为它分配一个**可用的Cluster IP**，而且在**Service的整个生命周期**内，它的**Cluster IP不会发生改变**。于是，服务发现这个棘手的问题在Kubernetes的架构里也得以轻松解决：只要用Service的Name与Service的Cluster IP地址做一个DNS域名映射即可完美解决问题。现在想想，这真是一个很棒的设计。
