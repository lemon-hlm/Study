...
另外，为了能够在**其他节点**上**恢复某个失败的节点**，这种集群中的**Pod需要挂接某种共享存储**，为了解决这个问题，Kubernetes从1.4版本开始引入了**PetSet**这个新的资源对象，并且在1.5版本时更名为**StatefulSet**，StatefulSet从本质上来说，可以看作Deployment/RC的一个特殊变种，它有如下特性。