...
另外，为了能够在**其他节点**上**恢复某个失败的节点**，这种集群中的**Pod需要挂接某种共享存储**，为了解决这个问题，Kubernetes从1.4版本开始引入了**PetSet**这个新的资源对象，并且在1.5版本时更名为**StatefulSet**，StatefulSet从本质上来说，可以看作**Deployment/RC**的一个**特殊变种**，它有如下特性。
...
Headless Service与普通Service的关键区别在于，它**没有Cluster IP**，如果解析Headless Service的DNS域名，则返回的是该Service对应的全部Pod的Endpoint列表。StatefulSet在Headless Service的基础上又为StatefulSet控制的每个Pod实例都创建了一个DNS域名，这个域名的格式为：